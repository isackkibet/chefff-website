...
export default function Toast({ id, type, message, onDismiss }: ToastProps) {
  ...
  return (
    ...
    >
      ...
      <p className="flex-1 text-sm font-medium text-[hsl(42_30%_94%)]">{message}</p>
      <button onClick={() => onDismiss(id)} className="shrink-0 opacity-60 hover:opacity-100 transition-opacity" aria-label="Dismiss">
        <X size={16} />
      </button>
    </div>
  )
