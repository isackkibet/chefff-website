...
export default function AdminBookingsPage() {
  ...
  async function patchBooking(
    id: string,
    updates: { status?: BookingStatus; notes?: string; quotedAmount?: number },
  ) {
    const res = await fetch("/api/admin/bookings", {
      method: "PATCH",
  ...
  async function saveQuote() {
    ...
    try {
      ...
      setSelected((prev) =>
        prev ? { ...prev, quotedAmount: amount, status: "QUOTED" } : null,
      );
      setQuoteAmount("");
    } catch {
                  ...
                  <thead>
                    <tr className="border-b border-[hsl(0_0%_16%)]">
                      {[
                        ...
                        <th
                          key={h}
                          className="px-5 py-3 text-left text-xs font-semibold text-[hsl(0_0%_45%)] uppercase tracking-wider whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-[hsl(0_0%_14%)]">
                    {filtered.length === 0 ? (
                      <tr>
                        <td
                          colSpan={8}
                          className="px-5 py-12 text-center text-[hsl(0_0%_45%)]"
                        >
                      ...
                      filtered.map((b) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3.5 text-[hsl(0_0%_65%)] text-center">
                            {b.guestCount}
                          </td>
                          <td className="px-5 py-3.5 text-[hsl(0_0%_65%)] whitespace-nowrap max-w-[140px] truncate">
                            {b.location}
                          </td>
        ...
        {selected && (
          <div>
            <div className="flex items-start justify-between gap-4 mb-5 pr-8">
              <div>
                <p className="font-mono text-xs text-[hsl(45_90%_52%)] mb-1">
                  #{selected.refNumber}
                </p>
            ...
            <div className="grid sm:grid-cols-2 gap-x-8 gap-y-3 mb-6 text-sm">
              {[
                ...
                {
                  label: "Date",
                  value: new Date(selected.eventDate).toLocaleDateString(
                    "en-KE",
                    {
                      weekday: "long",
                      day: "numeric",
            ...
            {selected.specialRequests && (
              <div className="mb-5 rounded-xl bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] p-3">
                ...
                <p className="text-sm text-[hsl(42_30%_85%)]">
                  {selected.specialRequests}
                </p>
              </div>
            )}
              ...
              <div className="mb-5 rounded-xl bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] p-4">
                <p className="text-sm font-semibold mb-3">
                  {selected.quotedAmount
                    ? `Current Quote: KES ${selected.quotedAmount.toLocaleString()}`
                    : "Send a Quote"}
                </p>
                <div className="flex gap-3">
                  ...
                  />
                  <Button variant="primary" size="sm" onClick={saveQuote}>
                    Send Quote
                  </Button>
                </div>
              </div>
