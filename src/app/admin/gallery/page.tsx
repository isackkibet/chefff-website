...
export default function AdminGalleryPage() {
  ...
  return (
    <AdminGuard>
          ...
          <div className="px-4 py-6 pb-28 sm:px-6 lg:p-8 lg:pb-8 max-w-7xl mx-auto">
            ...
            {images.length === 0 && (
              <div className="py-24 text-center">
                <ImageOff size={40} className="text-[hsl(0_0%_30%)] mx-auto mb-3" aria-hidden="true" />
      ...
      {/* Add image modal */}
      <Modal open={addOpen} onClose={() => setAddOpen(false)} aria-label="Add gallery image" className="max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input id="img-alt" value={form.alt} onChange={(e) => setForm({ ...form, alt: e.target.value })} className={inputClass} placeholder="Describe the image…" />
          </div>
          <div>
            <label htmlFor="img-caption" className="block text-sm font-medium mb-1.5">Caption (optional)</label>
            <input id="img-caption" value={form.caption} onChange={(e) => setForm({ ...form, caption: e.target.value })} className={inputClass} placeholder="Short display caption" />
          </div>
          <div>
            <label htmlFor="img-cat" className="block text-sm font-medium mb-1.5">Category *</label>
            <select id="img-cat" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as typeof form.category })} className={`${inputClass} cursor-pointer`}>
          ...
          <div className="flex gap-3 pt-1">
            <Button variant="primary" onClick={handleAdd} className="flex-1">Upload Image</Button>
