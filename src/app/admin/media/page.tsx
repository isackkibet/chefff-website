...
import { Clapperboard, Copy, Plus, Trash2, Video, UploadCloud } from 'lucide-react'
...
export default function AdminMediaPage() {
  ...
  async function loadVideos() {
    try {
      ...
      if (!res.ok) {
        toast('error', res.status === 503 ? 'Cloudinary is not configured.' : 'Failed to load videos.')
        return
      }
  ...
  async function handleUpload() {
    ...
    if (file.size > params.maxUploadBytes) {
      toast('error', `Video is too large. Maximum is ${formatBytes(params.maxUploadBytes)}.`)
      return
    }
  ...
  async function handleDelete(publicId: string) {
    try {
      const res = await fetch(`/api/admin/media?id=${encodeURIComponent(publicId)}`, { method: 'DELETE' })
      if (!res.ok) throw new Error()
      setDeleteId(null)
  ...
  return (
    <AdminGuard>
          ...
          <div className="px-4 py-6 pb-28 sm:px-6 lg:p-8 lg:pb-8 max-w-7xl mx-auto">
            ...
            {videos.length > 0 ? (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" role="list">
                {videos.map((video) => (
                  <div key={video.publicId} role="listitem" className="group rounded-2xl overflow-hidden bg-[hsl(0_0%_12%)] border border-[hsl(0_0%_18%)]">
                    ...
                    <div className="p-3">
                      <p className="text-xs font-medium text-[hsl(42_30%_94%)] leading-snug truncate" title={video.publicId}>
                      ...
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" className="flex-1 !px-2 text-xs" onClick={() => copyUrl(video.url)}>
              ...
              <div className="py-24 text-center">
                <Clapperboard size={40} className="text-[hsl(0_0%_30%)] mx-auto mb-3" aria-hidden="true" />
                <p className="text-[hsl(0_0%_40%)]">No videos yet. Upload one to get started.</p>
              </div>
            )}
          </div>
        </main>
      ...
      {/* Upload modal */}
      <Modal open={uploadOpen} onClose={() => { if (!uploading) setUploadOpen(false) }} aria-label="Upload video" className="max-w-md">
        <h2 className="font-display text-xl font-bold mb-1 pr-8">Upload Video</h2>
        <p className="text-sm text-[hsl(0_0%_50%)] mb-5">
          {maxBytes ? `Max size: ${formatBytes(maxBytes)}. Video files only.` : 'Video files only.'}
        </p>

        <label htmlFor="video-file" className="block text-sm font-medium mb-1.5">Video file *</label>
        <input
          id="video-file"
          ref={fileRef}
          type="file"
        ...
        <div className="flex gap-3 pt-5">
          <Button variant="primary" onClick={handleUpload} loading={uploading} className="flex-1">
            {!uploading && <Video size={16} aria-hidden="true" />} Upload
          </Button>
          <Button variant="outline" onClick={() => setUploadOpen(false)} disabled={uploading} className="flex-1">
            Cancel
          </Button>
        </div>
      </Modal>
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} aria-label="Confirm delete" className="max-w-sm">
        <div className="text-center py-2">
          ...
          <p className="text-sm text-[hsl(0_0%_50%)] mb-6">This will remove it from Cloudinary. This action cannot be undone.</p>
          <div className="flex gap-3">
            <Button variant="ghost" onClick={() => setDeleteId(null)} className="flex-1">Cancel</Button>
            <Button onClick={() => deleteId && handleDelete(deleteId)} className="flex-1 bg-[hsl(0_72%_51%)] text-white hover:bg-[hsl(0_72%_44%)]">
              Delete
            </Button>
          </div>
        </div>
