...
export default function MenuClient() {
  ...
  const filtered = menuItems.filter((item) => {
    ...
    const dietMatch = dietary.length === 0 || dietary.every((d) => item.dietary.includes(d))
  ...
  return (
    <section className="pb-24 px-4 sm:px-6 lg:px-8" aria-label="Menu items">
      <div className="mx-auto max-w-7xl">
        ...
        {/* Category filter */}
        <div className="flex flex-wrap gap-2 justify-center mb-6" role="tablist" aria-label="Menu categories">
          {categories.map((cat) => (
            <button
              key={cat} role="tab" aria-selected={category === cat}
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-10" role="group" aria-label="Dietary filters">
          ...
          {dietary.length > 0 && (
            <button onClick={() => setDietary([])} className="text-xs text-[hsl(0_0%_45%)] hover:text-[hsl(42_30%_94%)] transition-colors px-2">
              Clear filters
            </button>
          )}
        </div>

        <p className="text-sm text-[hsl(0_0%_45%)] text-center mb-8" aria-live="polite" aria-atomic="true">
          Showing {filtered.length} {filtered.length === 1 ? 'dish' : 'dishes'}
        </p>

        {filtered.length === 0 ? (
          <div className="py-24 text-center">
            <p className="text-[hsl(0_0%_45%)] text-lg">No dishes match your current filters.</p>
            <button onClick={() => { setCategory('All'); setDietary([]) }} className="mt-4 text-[hsl(45_90%_52%)] hover:underline text-sm">
              Clear all filters
            </button>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" role="list">
            {filtered.map((dish) => (
              <article key={dish.id} role="listitem" className="group overflow-hidden rounded-2xl bg-[hsl(0_0%_12%)] border border-[hsl(0_0%_18%)] card-hover flex flex-col">
                ...
                <div className="p-4 flex flex-col flex-1">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <h3 className="font-display font-semibold text-sm leading-snug">{dish.name}</h3>
                    <Badge variant="gold">{dish.category}</Badge>
                  </div>
                  <p className="text-xs text-[hsl(0_0%_55%)] leading-relaxed mb-3 line-clamp-2 flex-1">{dish.description}</p>
                  {dish.dietary.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {dish.dietary.map((d) => <DietaryBadge key={d} tag={d} />)}
                    </div>
                  )}
      ...
      {/* Dish detail modal */}
      <Modal open={!!selected} onClose={() => setSelected(null)} aria-label={selected ? `Details for ${selected.name}` : undefined} className="max-w-lg">
        {selected && (
          <div>
            ...
            <div className="flex items-start justify-between gap-3 mb-2">
              ...
              <span className="font-bold text-lg text-[hsl(45_90%_52%)] shrink-0">KES {selected.price.toLocaleString()}</span>
            </div>
            <Badge variant="gold" className="mb-3">{selected.category}</Badge>
            <p className="text-sm text-[hsl(0_0%_65%)] leading-relaxed mb-4">{selected.description}</p>
            <div className="grid grid-cols-2 gap-4 text-sm mb-4">
              <div>
                <p className="font-semibold text-[hsl(42_30%_94%)] mb-1">Ingredients</p>
                <p className="text-[hsl(0_0%_55%)]">{selected.ingredients.join(', ')}</p>
              </div>
              {selected.allergens.length > 0 && (
                <div>
                  <p className="font-semibold text-[hsl(42_30%_94%)] mb-1">Allergens</p>
                  <p className="text-[hsl(0_72%_65%)]">{selected.allergens.join(', ')}</p>
                </div>
              )}
            </div>
            {selected.dietary.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-5">
                {selected.dietary.map((d) => <DietaryBadge key={d} tag={d} />)}
              </div>
            )}
            <ButtonLink href="/book" variant="primary" className="w-full justify-center">
