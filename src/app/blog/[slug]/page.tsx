  ...
  return (
    <>
      <div className="pt-28 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto mb-8">
        <Link href="/blog" className="inline-flex items-center gap-2 text-sm text-[hsl(0_0%_55%)] hover:text-[hsl(45_90%_52%)] transition-colors">
      ...
      <article className="pb-24 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-8 leading-tight">{post.title}</h1>
          ...
          <div className="mt-12 pt-8 border-t border-[hsl(0_0%_18%)] flex items-center gap-4">
            <div className="relative size-14 rounded-full overflow-hidden border-2 border-[hsl(45_90%_52%/0.3)]">
              <Image src="https://images.unsplash.com/photo-1577219491135-ce391730fb2c?w=120&q=80" alt={post.author} fill className="object-cover" />
            </div>
            <div>
              <p className="font-semibold text-[hsl(42_30%_94%)]">{post.author}</p>
              <p className="text-sm text-[hsl(0_0%_50%)]">Private Chef · Nairobi, Kenya</p>
            </div>
          </div>
        </div>
        ...
        {related.length > 0 && (
          <div className="mt-20 max-w-4xl mx-auto">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {related.map((r) => (
                <Link key={r.id} href={`/blog/${r.slug}`} className="group flex gap-4 rounded-2xl bg-[hsl(0_0%_12%)] border border-[hsl(0_0%_18%)] p-4 card-hover">
                  <div className="relative w-24 h-24 rounded-xl overflow-hidden shrink-0">
                    <Image src={r.image} alt={r.title} fill className="object-cover" sizes="96px" />
                  </div>
                  <div>
                    <Badge variant="muted" className="mb-2">{r.category}</Badge>
                    <h3 className="font-semibold text-sm leading-snug group-hover:text-[hsl(45_90%_52%)] transition-colors">{r.title}</h3>
                  </div>
                </Link>
              ))}
            </div>
